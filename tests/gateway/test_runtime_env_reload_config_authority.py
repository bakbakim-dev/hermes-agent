...
def test_reload_runtime_env_preserves_config_timezone(tmp_path: Path, monkeypatch) -> None:
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        yaml.safe_dump({"timezone": "America/Edmonton"}),
        encoding="utf-8",
    )
    (hermes_home / ".env").write_text(
        "HERMES_TIMEZONE=UTC\nHERMES_LOCAL_TIMEZONE=UTC\nTZ=UTC\n",
        encoding="utf-8",
    )

    monkeypatch.setattr(gateway_run, "_hermes_home", hermes_home)
    monkeypatch.setenv("HERMES_TIMEZONE", "UTC")
    monkeypatch.setenv("HERMES_LOCAL_TIMEZONE", "UTC")
    monkeypatch.setenv("TZ", "UTC")

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["HERMES_TIMEZONE"] == "America/Edmonton"
    assert os.environ["HERMES_LOCAL_TIMEZONE"] == "America/Edmonton"
    assert os.environ["TZ"] == "America/Edmonton"
