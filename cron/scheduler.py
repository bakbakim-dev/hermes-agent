...
def _extract_quoted_delivery_body(text: str) -> str:
    stripped = text.strip()
    if not stripped:
        return ""
    for quote in ('"', "'"):
        first = stripped.find(quote)
        last = stripped.rfind(quote)
        if first != -1 and last > first:
            candidate = stripped[first + 1:last].strip()
            if candidate:
                return candidate
    return ""
...
def _sanitize_cron_delivery_content(content: str) -> str:
    ...
    if not text:
        return ""
    if SILENT_MARKER in text.upper():
        return SILENT_MARKER
    lower = text.lower()
    if not any(pattern in lower for pattern in _CRON_DELIVERY_META_PATTERNS):
        return text

    extracted = _extract_quoted_delivery_body(text)
    if extracted:
        return _strip_outer_quotes(extracted)
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        # retained as a legacy fallback for older CLI configs.
